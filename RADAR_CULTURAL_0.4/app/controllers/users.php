...
if (isset($_POST['forgot-password-btn'])){
	...
	$remetente = "FROM: radarculturalavisos@gmail.com";
	$email = $_POST['email'];
	...
	if (empty($user)) {
		array_push($errors, 'Email não existente');

	} else {

		$nova_senha = substr(md5(time()), 0, 6);
		$nova_senha_criptografada = md5(md5($nova_senha));

		if (mail($email, "Sua nova senha!", "Sua nova senha: ".$nova_senha, $remetente)) {
			$count = recuperarSenha($table, $email, $nova_senha_criptografada);
		} else{
			array_push($errors, 'errooo');
		}
